...
This workspace contains the operational compiled core runtime engine for the refactored, high-performance ERPNext architecture built in Rust. It delivers low-latency dual-protocol networking, metadata registry schemas, a zero-copy local storage layer, and a robust document business logic layer.
...
graph TD
    ...
    subgraph frappe-framework [document business logic]
        Lifecycle[Document Lifecycle & Transitions]
        Permissions[Security: RBAC, RLS, FLS]
        Scripting[Rhai Scripting Sandbox]
    end
    ...
    Routes --> frappe-framework
    frappe-framework -->|Query / DDL| SurrealDB[(SurrealDB Instance)]
...
### 1. `frappe-framework` (Main Library)
Provides the core business logic and document handling layer:
- **Document Lifecycle (`lifecycle.rs`)**: Enforces document state transitions (Draft -> Submitted -> Cancelled). Ensures immutability of submitted records and manages validation constraints.
- **Permissions (`permissions.rs`)**: Implements a multi-layered security model including Role-Based Access Control (RBAC), Row-Level Security (RLS) via user constraints, and Field-Level Security (FLS) to redact sensitive data.
- **Scripting Engine (`scripting.rs`)**: Incorporates a sandboxed Rhai scripting environment, allowing custom business logic and dynamic document mutation with safe, rate-limited execution and isolated database access.
...
- **Rhai 1.19.0**: Embedded scripting engine for safe, dynamic business logic execution.